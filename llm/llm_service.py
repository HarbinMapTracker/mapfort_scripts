...
class LLMService:
    ...
    def _build_prompt(self, driver_data: Dict[str, Any]) -> str:
        ...
        recent_data = driver_data.get("recent_driving", {})
          # 构建简洁的中文提示，明确区分七天数据和当日数据
        prompt = f"""
作为驾驶安全顾问，请根据以下数据提供简短休息建议，并以JSON5格式返回：
...
{{
  ...
  recommendation: "休息建议文本"  // 50字以内，适合TTS朗读的友好建议
}}

请使用友好语气，给出简短实用的建议（这个建议应该适合口播，而非书面）。

例如: {{ needs_rest: false, recommendation: "您今日已驾驶8h, 长途订单刚刚结束，请休息片刻。注意上次连开近5小时，下次长途请每2小时休息15分。" }}
"""
        return prompt
    def _get_recommendation(self, prompt: str) -> Dict[str, Any]:
        ...
        try:            
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "你是驾驶安全顾问，提供简短的休息建议，适合TTS播报。回复应简洁明了，控制在50字以内。请以JSON5格式返回，包含needs_rest(布尔值)和recommendation(字符串)字段。"},
